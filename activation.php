...
if (isset($_GET['id']) && isset($_GET['u']) && isset($_GET['hash'])) {
    ...
    if($id == "" || strlen($u) < 3 || ($hash) == ""){
        // Log this issue into a text file and email details to yourself for record keeping.
    ...
    if($numrows == 0){
        ...
        $stmt->execute(array(':initiator' => $u, ':target' => $admin, ':action' => $action, ':detail' => $detail));
        // Exit
        exit();
    }
} else {
