...
$sql_trans = "SELECT businessdetails.id, businessEmail, businessdetails.username, businessName, address, contactName, businessdetails.mobile, users.activated, agent_business_alloc.agentUsername FROM businessdetails INNER JOIN users ON businessdetails.username=users.username LEFT OUTER JOIN agent_business_alloc ON agent_business_alloc.businessUsername = businessdetails.username WHERE agent_business_alloc.agentUsername=:logusername AND users.activated='1' ";
...
echo '</div>';
  ...
  echo '<strong>You don\'t have any business in your portfolio yet.</strong>';
  echo '</div>';
  echo '</div>';
}
?>
