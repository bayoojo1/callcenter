...
if($isMessage && $isApproved) {
  ...
  $sql_trans = "SELECT salesleads.id, salesleads.agentUsername, salesleads.fsrUsername, salesleads.businessUsername, leadMobile, infoRequested, leadDate, agentdetails.agent_firstname, users.avatar FROM salesleads LEFT OUTER JOIN agentdetails ON salesleads.agentUsername=agentdetails.username LEFT OUTER JOIN users ON salesleads.agentUsername=users.username WHERE businessUsername=:logusername ORDER BY id DESC";
  ...
  $sql = "SELECT businessdetails.businessName, businessdetails.agentUsername, agentdetails.agent_firstname, users.avatar FROM businessdetails INNER JOIN agentdetails ON agentdetails.username=businessdetails.agentUsername INNER JOIN users ON users.username=businessdetails.agentUsername WHERE businessdetails.username=:logusername";
  $stmt = $db_connect->prepare($sql);
  $stmt->bindParam(':logusername', $log_username, PDO::PARAM_STR);
  $stmt->execute();
  foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
