...
function evalLoggedUser($db_connect,$id,$e,$u,$p) {
    include("mysqli_connect.php");
    $sql = "SELECT ip FROM users WHERE id=:id AND email=:email AND username=:username AND password=:password AND activated='1' LIMIT 1";
...
if(isset($_SESSION["userid"]) && isset($_SESSION["email"]) && isset($_SESSION["username"]) && isset($_SESSION["password"])) {
    $log_id = preg_replace('#[^0-9]#', '', $_SESSION['userid']);
    $log_email = preg_replace('#[^a-z0-9.@_]#i', '', $_SESSION['email']);
    $log_username = preg_replace('#[^a-z0-9.@_]#i', '', $_SESSION['username']);
    $log_password = $_SESSION['password'];
    // Verify the user
    $user_ok = evalLoggedUser($db_connect,$log_id,$log_email,$log_username,$log_password);
} else if(isset($_COOKIE["id"]) && isset($_COOKIE["mail"]) && isset($_COOKIE["user"]) && isset($_COOKIE["pass"])){
    $_SESSION['userid'] = preg_replace('#[^0-9]#', '', $_COOKIE['id']);
    $_SESSION['email'] = preg_replace('#[^a-z0-9.@_]#i', '', $_COOKIE['mail']);
